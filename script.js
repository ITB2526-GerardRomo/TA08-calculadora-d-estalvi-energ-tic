...
function compute() {
  ...
  const totalSavedRaw  = totalCostBase - totalCostFinal;
  const totalPctRaw    = totalCostBase > 0 ? (totalSavedRaw / totalCostBase) * 100 : 0;

  /* Cap real al 30%: si els sliders passen del límit, escalem l'estalvi */
  const CAP = 30;
  const scaleFactor    = totalPctRaw > CAP ? CAP / totalPctRaw : 1;
  const totalSaved     = totalSavedRaw * scaleFactor;
  const totalPct       = Math.min(totalPctRaw, CAP);
  const co2Saved       = (kwhSaved * scaleFactor) * BASE.elect.co2PerKwh;
...
/** (Mini barres estacionals eliminades) */
